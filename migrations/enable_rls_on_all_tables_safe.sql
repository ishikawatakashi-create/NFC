...
-- 安全版: site_idカラムが存在しないテーブルでもエラーが発生しないように修正
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sites') THEN
    ...
    EXECUTE format('CREATE POLICY "Admins can manage sites" ON public.sites
      FOR ALL
      USING (
        public.is_admin()
...
BEGIN
  IF EXISTS (
    ...
    EXECUTE format('CREATE POLICY "Admins can manage role_based_access_times" ON public.role_based_access_times
      FOR ALL
      USING (
        public.is_admin()
        AND site_id = public.get_admin_site_id()
...
BEGIN
  IF EXISTS (
    ...
    EXECUTE format('CREATE POLICY "Admins can manage card_tokens" ON public.card_tokens
      FOR ALL
      USING (
        public.is_admin()
...
BEGIN
  IF EXISTS (
    ...
    EXECUTE format('CREATE POLICY "Admins can manage role_based_bonus_thresholds" ON public.role_based_bonus_thresholds
      FOR ALL
      USING (
        public.is_admin()
        AND site_id = public.get_admin_site_id()
...
BEGIN
  IF EXISTS (
    ...
    EXECUTE format('CREATE POLICY "Admins can manage class_based_bonus_thresholds" ON public.class_based_bonus_thresholds
      FOR ALL
      USING (
        public.is_admin()
        AND site_id = public.get_admin_site_id()
...
BEGIN
  IF EXISTS (
    ...
    EXECUTE format('CREATE POLICY "Admins can manage parents" ON public.parents
      FOR ALL
      USING (
        public.is_admin()
        AND site_id = public.get_admin_site_id()
...
BEGIN
  IF EXISTS (
    ...
    IF EXISTS (
      ...
      EXECUTE format('CREATE POLICY "Admins can manage line_link_tokens" ON public.line_link_tokens
        FOR ALL
        USING (
          public.is_admin()
          AND site_id = public.get_admin_site_id()
...
BEGIN
  IF EXISTS (
    ...
    EXECUTE format('CREATE POLICY "Admins can manage point_transactions" ON public.point_transactions
      FOR ALL
      USING (
        public.is_admin()
        AND site_id = public.get_admin_site_id()
...
BEGIN
  IF EXISTS (
    ...
    EXECUTE format('CREATE POLICY "Admins can manage access_logs" ON public.access_logs
      FOR ALL
      USING (
        public.is_admin()
        AND site_id = public.get_admin_site_id()
...
BEGIN
  IF EXISTS (
    ...
    EXECUTE format('CREATE POLICY "Admins can manage line_notification_logs" ON public.line_notification_logs
      FOR ALL
      USING (
        public.is_admin()
        AND site_id = public.get_admin_site_id()
...
BEGIN
  IF EXISTS (
    ...
    EXECUTE format('CREATE POLICY "Admins can manage point_settings" ON public.point_settings
      FOR ALL
      USING (
        public.is_admin()
        AND site_id = public.get_admin_site_id()

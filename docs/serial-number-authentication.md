...
**注意**: Web NFC APIの制限により、SuicaなどのFeliCaカードではシリアル番号を自動取得できません。手動登録が必要です。

**対応カード:**
- ✅ NTAG213 / NTAG215 / NTAG216（自動読み取り可能）
- ✅ Unit Linkカード（自動読み取り可能）
- ⚠️ Suica / PASMO などの交通系ICカード（手動登録が必要）
- ❌ マイナンバーカード（読み取り不可）
- ❌ パスポート（読み取り不可）

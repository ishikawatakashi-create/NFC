...
### セットアップ時
1. `QUICK_START.md` - クイックスタート
2. `admin-authentication-setup.md` - 管理者認証設定
3. `ngrok-setup-windows.md` - ngrokセットアップ
4. `line-notification-setup-guide.md` - LINE通知設定

### トラブルシューティング時
1. `TROUBLESHOOTING_503.md` - 503エラー
2. `TESTING_STEPS.md` - テスト手順
3. `debugging-nfc-serial-number.md` - NFCデバッグ
4. `vercel-logs-guide.md` - Vercelログ確認

### 機能実装時
1. `line-integration-flow.md` - LINE連携フロー
2. `auto-exit-feature.md` - 自動退室機能
3. `nfc-card-registration-guide.md` - NFCカード登録
4. `kiosk-screen-guide.md` - キオスク画面

...
```
1. 親御さんがLINE公式アカウントに「カード紐づけを開始します」とメッセージを送信、またはリッチメニューの「カード紐づけ」ボタンをタップ
   ↓
...
1. **トークン生成**
   - LINE Webhookで「カード紐づけを開始します」メッセージまたはpostbackイベントを受信
   - 32バイトのランダムトークンを生成
...
- 正しいメッセージ（「カード紐づけを開始します」など）を送信しているか、またはリッチメニューのボタンをタップしているか
- Webhookが有効になっているか
- サーバーログを確認
